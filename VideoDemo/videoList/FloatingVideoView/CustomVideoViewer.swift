...
class CustomVideoViewer: UIView {
  let clips: [VideoClip]
  ...
  func addGestureRecognizers() {
    // 1
    let tap = UITapGestureRecognizer(target: self, action: #selector(CustomVideoViewer.wasTapped))
    let doubleTap = UITapGestureRecognizer(target: self,
                                           action: #selector(CustomVideoViewer.wasDoubleTapped))
